  ...
  return (
    <div className='flex flex-col mt-4'>
        {orders?.length > 0 ? (
            orders.map((item) => {
                return (
                 <div key={item.id} onClick={() => setShowDetails(!showDetails)} className='cursor-pointer border-b border-[var(--border)] py-4 last:border-0 '>
                    <div className='grid grid-cols-[1fr_1fr_auto] sm:grid-cols-[1fr_1fr_1fr_1fr_auto] gap-4 items-center'>
                        ...
                        <FaChevronRight className={`transition-transform duration-200 ${showDetails ? 'rotate-90' : ''}`} />
                    </div>
                    <div className={`flex flex-col gap-4 overflow-hidden transition-all duration-300 ${showDetails ? 'max-h-[1000px]' : 'max-h-0'}`}>
